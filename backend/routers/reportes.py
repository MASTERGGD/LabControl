...
def _get_historial_alumno(matricula: str, db: Session) -> dict:
    """Recopila el historial completo de asistencia de un alumno por matrícula."""
    asigs = db.query(AsignacionPC).filter(
        AsignacionPC.alumno_matricula == matricula
    ).all()

    # Datos del catálogo (entrada más reciente)
    cat = db.query(CatalogoAlumno).filter(
        CatalogoAlumno.matricula == matricula
    ).order_by(CatalogoAlumno.id.desc()).first()

    if not asigs:
        nombre = cat.nombres + " " + cat.apellido_paterno if cat else "—"
        return {
            "alumno": {
                "matricula":    matricula,
                "nombre":       nombre,
    ...
    # Cargar sesiones, laboratorios y docentes en batch
    sesion_ids = list({a.sesion_id for a in asigs})
    sesiones_map = {s.id: s for s in db.query(SesionClase).filter(SesionClase.id.in_(sesion_ids)).all()}
    lab_ids  = list({s.laboratorio_id for s in sesiones_map.values()})
    labs_map = {l.id: l for l in db.query(Laboratorio).filter(Laboratorio.id.in_(lab_ids)).all()}
    doc_ids  = list({s.docente_id for s in sesiones_map.values()})
    docs_map = {u.id: u for u in db.query(Usuario).filter(Usuario.id.in_(doc_ids)).all()}
    pc_ids   = list({a.computadora_id for a in asigs})
    pcs_map  = {p.id: p for p in db.query(Computadora).filter(Computadora.id.in_(pc_ids)).all()} if pc_ids else {}
    ...
    # Nombre preferido: del catálogo si existe, si no del primer registro
    nombre_cat = f"{cat.nombres} {cat.apellido_paterno} {cat.apellido_materno}".strip() if cat else None
    nombre_asig = asigs[0].alumno_nombre

    return {
        "alumno": {
            "matricula":    matricula,
            "nombre":       nombre_cat or nombre_asig,
            "carrera":      cat.carrera      if cat else None,
            "cuatrimestre": cat.cuatrimestre if cat else None,
            "grupo":        cat.grupo        if cat else None,
            "periodo":      cat.periodo      if cat else None,
...
def _build_historial_excel(data: dict) -> io.BytesIO:
    """Genera el Excel del historial de un alumno."""
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Historial"
    ws.sheet_view.showGridLines = False
    ...
    t.fill      = PatternFill("solid", fgColor=AZUL_OSC)
    ...
    HDR_ROW = len(meta) + 3
    HDRS = [
        ("#",         5), ("Fecha",       12), ("Materia",     30),
        ("Grupo",     8), ("Docente",     24), ("Laboratorio", 22),
        ("PC",        10), ("Horas",       10), ("Estado",      14),
    ]
    ...
    for r, h in enumerate(historial, HDR_ROW + 1):
        fill_obj = PatternFill("solid", fgColor=GRIS_ROW) if r % 2 == 0 else None
        vals = [
            r - HDR_ROW,
            h.get("fecha") or "—",
            h.get("materia") or "—",
    ...
    c1.fill = PatternFill("solid", fgColor=AZUL_MED)
    ...
    c2.fill = PatternFill("solid", fgColor="1B5E20")
...
def historial_alumno_json(
    ...
    """Devuelve el historial completo de sesiones asistidas por un alumno."""
    if current_user.rol == RolUsuario.DOCENTE:
        raise HTTPException(403, "Acceso restringido a administradores")
    return _get_historial_alumno(matricula, db)
...
def historial_alumno_excel(
    ...
    """Descarga el historial del alumno como archivo Excel."""
    if current_user.rol == RolUsuario.DOCENTE:
        raise HTTPException(403, "Acceso restringido a administradores")
    data     = _get_historial_alumno(matricula, db)
