...
export default function OfflineStatus({ enabled }) {
  ...
  return <>
    ...
    {abierto && <div className="fixed inset-0 z-[100] flex items-end justify-center bg-slate-950/70 p-0 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={() => setAbierto(false)}>
      <section role="dialog" aria-modal="true" aria-labelledby="titulo-sincronizacion" onMouseDown={event => event.stopPropagation()} className="max-h-[90vh] w-full overflow-y-auto rounded-t-2xl border border-slate-600 bg-slate-900 p-5 text-slate-100 shadow-2xl sm:max-w-2xl sm:rounded-2xl">
        <div className="flex items-start justify-between gap-4"><div><h2 id="titulo-sincronizacion" className="text-lg font-bold">Capturas de este dispositivo</h2><p className="text-sm text-slate-400">El servidor conserva la autoridad. El plazo de aceptación automática es de 7 días.</p></div><button onClick={() => setAbierto(false)} aria-label="Cerrar" className="text-2xl text-slate-400">×</button></div>
        {!operations.length ? <p className="mt-5 rounded-xl bg-slate-800 p-4">No hay capturas pendientes.</p> : <div className="mt-5 space-y-3">{operations.map(item => {
        ...
        <div className="mt-5 flex flex-wrap justify-end gap-2"><button onClick={() => setAbierto(false)} className="rounded-lg px-4 py-2 text-sm text-slate-300">Cerrar</button>{canSync && pending > 0 && <button disabled={syncing} onClick={sync} className="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{syncing ? 'Sincronizando…' : 'Sincronizar ahora'}</button>}</div>
      </section>
    </div>}
  </>;
}
