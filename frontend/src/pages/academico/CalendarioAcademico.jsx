  ...
  return (
    <AdminLayout>
      <div className="space-y-5">
        ...
        {puedeAdministrar && (
          <section className="glass rounded-2xl p-4"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-wider text-amber-300">Cierre académico del cuatrimestre</p><h2 className="mt-1 font-semibold text-white">{cuatrimestreCerrado ? '✓ Cierre completado' : cierre?.estado || 'Sin configurar'}</h2><p className="mt-1 text-xs text-slate-400">{cuatrimestreCerrado ? `Cuatrimestre y calendario en solo consulta${cierre.cerrado_en ? ` · Cerrado el ${new Date(`${cierre.cerrado_en}Z`).toLocaleString('es-MX')}` : ''}.` : cierre ? `${cierre.confirmadas}/${cierre.total_cargas} cargas confirmadas · ${cierre.con_pendientes} con clases abiertas` : 'Activa el pre-cierre cuando terminen las actividades académicas.'}</p>{cierre?.laboratorios && <p className={`mt-1 text-xs ${cierre.laboratorios.sesiones_abiertas ? 'text-amber-300' : 'text-emerald-300'}`}>Laboratorios: {cierre.laboratorios.reservaciones} reservas · {cierre.laboratorios.sesiones_abiertas} sesiones abiertas{cierre.estado === 'CERRADO' ? ' · agenda archivada' : ''}</p>}</div><div className="flex flex-wrap gap-2">{!cuatrimestreCerrado && <>
              {(!cierre || cierre.estado === 'ACTIVO') && <button disabled={guardando} onClick={() => configurarCierre('PRECIERRE')} className="rounded-lg bg-amber-600 px-3 py-2 text-xs font-semibold text-white disabled:opacity-40">Iniciar pre-cierre</button>}
              <button disabled={guardando} onClick={abrirConfirmacion} className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white disabled:opacity-40">{cierre?.estado === 'CONFIRMACION' ? 'Ajustar fechas de confirmación' : 'Abrir confirmación'}</button>
              {cierre && <button disabled={guardando || cierre.laboratorios?.sesiones_abiertas > 0 || cierre.confirmadas < cierre.total_cargas} title="Cierra el cuatrimestre y archiva su calendario" onClick={() => abrirAccion('cerrar')} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white disabled:opacity-40">Cerrar cuatrimestre</button>}
      ...
      {accion && <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-sm" onKeyDown={e => { if (e.key === 'Escape' && !guardando) setAccion(null); }}>
        <form role="dialog" aria-modal="true" aria-labelledby="titulo-accion-calendario" onSubmit={confirmarAccion} className="w-full max-w-lg rounded-2xl border p-6 shadow-2xl" style={{ background: 'var(--main-bg)', color: 'var(--main-text)', borderColor: 'var(--topbar-border)' }}>
          ...
          <h2 id="titulo-accion-calendario" className="mt-2 text-xl font-bold">{accion === 'cerrar' ? 'Cerrar cuatrimestre y calendario' : accion === 'borrador' ? 'Volver el calendario a borrador' : 'Cancelar actividad del calendario'}</h2>
          <p className="mt-3 text-sm leading-6 opacity-80">{accion === 'cerrar' ? 'Se cerrará la operación académica, se archivará el calendario y la agenda de laboratorio. No se podrán crear ni activar materias. Los registros y el historial se conservarán; esta acción no elimina alumnos ni promueve grupos.' : accion === 'borrador' ? 'Las reglas del calendario dejarán de aplicarse hasta que vuelvas a publicarlo. Esta acción no cierra el cuatrimestre.' : 'La actividad dejará de aplicarse. El registro se conservará en el historial.'}</p>
          <label className="mt-5 block text-sm">{accion === 'cerrar' ? 'Observaciones del cierre (opcional)' : 'Motivo del cambio *'}<textarea autoFocus required={accion !== 'cerrar'} minLength={accion === 'cerrar' ? undefined : 5} maxLength={500} value={motivoAccion} onChange={e => setMotivoAccion(e.target.value)} className="input-dark mt-2 min-h-24" /></label>
          ...
          <div className="mt-6 flex justify-end gap-3"><button type="button" disabled={guardando} onClick={() => setAccion(null)} className="rounded-xl border px-4 py-2 text-sm disabled:opacity-40">Cancelar</button><button disabled={guardando} type="submit" className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-40">{guardando ? 'Procesando…' : accion === 'cerrar' ? 'Confirmar cierre' : 'Confirmar cambio'}</button></div>
        </form>
      </div>}
