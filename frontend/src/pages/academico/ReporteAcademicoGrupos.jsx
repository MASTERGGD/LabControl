...
export default function ReporteAcademicoGrupos() {
  ...
  const gruposFiltrados = useMemo(() => {
    ...
    return termino ? grupos.filter((g) => normalizar(`${g.nombre} ${g.carrera} ${g.turno || ''} ${g.generacion || ''}`).includes(termino)) : grupos;
  }, [grupos, busqueda]);
  ...
  const materiasSinRegistro = useMemo(() => (datos?.materias || []).filter((m) => (
    m.sesiones === 0 && m.sesiones_programadas > 0
  )), [datos]);
  const resumenExtemporaneas = useMemo(() => {
    const sesiones = (datos?.sesiones_especiales || []).filter((s) => s.extemporanea);
    if (!sesiones.length) return null;
    const porMotivo = sesiones.reduce((acc, sesion) => {
      const motivo = sesion.motivo_extemporaneo?.trim() || 'Sin motivo indicado';
      const clave = normalizar(motivo);
      if (!acc[clave]) acc[clave] = { motivo, total: 0, docentes: new Set() };
      acc[clave].total += 1;
      if (sesion.docente) acc[clave].docentes.add(sesion.docente);
      return acc;
    }, {});
    const principal = Object.values(porMotivo).sort((a, b) => b.total - a.total)[0];
    if (!principal || principal.total < 2) return null;
    return `${principal.total} de ${sesiones.length} registros extemporáneos se relacionan con “${principal.motivo}” y se concentran en ${principal.docentes.size} ${principal.docentes.size === 1 ? 'docente' : 'docentes'}.`;
  }, [datos]);
  ...
  return <AdminLayout><div className="space-y-5">
    ...
    <section className="glass space-y-4 rounded-2xl p-5">
      <div>
        <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
          <label className="min-w-[260px] flex-1 text-sm font-semibold text-slate-300">Buscar grupos<input type="search" value={busqueda} onChange={(e) => setBusqueda(e.target.value)} placeholder="Grupo, carrera, generación o turno" className="input-dark mt-1" /></label>
    ...
    {datos && <>
      ...
      {datos.resumen.materias === 0 ? <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-5 text-amber-100"><h2 className="font-semibold">No hay información académica para reportar</h2><p className="mt-1 text-sm">El periodo {datos.periodo.clave} no tiene materias configuradas para {datos.grupos.map((g) => `${g.nombre} · ${g.carrera}`).join('; ')}. Configure la carga académica antes de evaluar sesiones, asistencia o indicadores.</p></div> : <>
        {materiasSinRegistro.length > 0 && <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-500/35 bg-amber-500/10 px-4 py-3 text-sm text-amber-100"><p><b>Requiere atención:</b> {materiasSinRegistro.map((m) => m.materia).join(', ')} {materiasSinRegistro.length === 1 ? 'no tiene sesiones registradas' : 'no tienen sesiones registradas'} hasta la fecha de corte.</p><a href="#detalle-materias" className="shrink-0 font-semibold text-amber-200 underline underline-offset-2">Ver {materiasSinRegistro.length === 1 ? 'materia' : 'materias'}</a></div>}
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{[['Alumnos',datos.resumen.alumnos],['Materias',datos.resumen.materias],['Sesiones registradas',`${datos.resumen.sesiones} de ${datos.resumen.sesiones_programadas}`],['Alumnos con indicador académico',datos.resumen.alumnos_atencion]].map(([k,v])=><div key={k} className="glass rounded-xl p-4"><p className="text-2xl font-bold text-white">{v}</p><p className="text-xs text-slate-300">{k}</p></div>)}</div>
      ...
      <section className="glass overflow-hidden rounded-2xl"><header className="border-b border-white/10 px-5 py-4"><h2 className="font-semibold text-white">Comparación de grupos</h2><p className="text-xs text-slate-300">Una fila por grupo para identificar diferencias de configuración y seguimiento.</p></header><div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-sm"><thead className="bg-white/[0.025] text-xs uppercase text-slate-400"><tr><th className="px-5 py-3">Grupo</th><th>Generación</th><th>Alumnos</th><th>Materias</th><th>Sesiones</th><th>Cobertura</th><th>Asistencia</th><th>Alumnos con indicador</th><th>Estado</th></tr></thead><tbody className="divide-y divide-white/5">{datos.grupos.map((g)=><tr key={g.id}><td className="px-5 py-3"><b className="text-white">{g.nombre}</b><small className="block max-w-xs text-slate-400">{g.carrera}</small></td><td><span className="rounded-full bg-blue-500/10 px-2 py-1 text-xs font-semibold text-blue-300">{g.generacion}</span></td><td>{g.alumnos}</td><td>{g.materias}</td><td>{g.sesiones}/{g.sesiones_programadas}</td><td>{pct(g.cobertura)}</td><td>{g.asistencia_detalle.publicable ? pct(g.asistencia) : 'Muestra insuficiente'}</td><td>{g.alumnos_atencion}</td><td><span className={`rounded-full px-2 py-1 text-xs font-semibold ${g.materias === 0 ? 'bg-amber-500/15 text-amber-300' : g.sesiones === 0 ? 'bg-blue-500/15 text-blue-300' : 'bg-emerald-500/15 text-emerald-300'}`}>{g.materias === 0 ? 'Sin carga académica' : g.sesiones === 0 ? 'Sin sesiones registradas' : 'Con información'}</span></td></tr>)}</tbody></table></div></section>
      {datos.materias.length > 0 && <section id="detalle-materias" className="glass scroll-mt-4 overflow-hidden rounded-2xl"><header className="border-b border-white/10 px-5 py-4"><h2 className="font-semibold text-white">Detalle de materias</h2><p className="text-xs text-slate-300">Información académica registrada por grupo y docente. La cobertura indica cuántas sesiones esperadas ya tienen registro.</p></header><div className="grid gap-4 p-4 lg:grid-cols-2">{datos.grupos.filter((g) => datos.materias.some((m) => m.grupo_id === g.id)).map((g)=><article key={g.id} className="overflow-hidden rounded-xl border border-white/10"><header className="border-b border-white/10 px-4 py-3"><h3 className="font-semibold text-white">{g.nombre}</h3><p className="text-xs text-slate-400">{g.carrera}</p></header><div className="divide-y divide-white/5">{datos.materias.filter((m)=>m.grupo_id===g.id).map((m)=><div key={`${m.materia}-${m.docente}`} className={`px-4 py-3 ${m.sesiones === 0 && m.sesiones_programadas > 0 ? 'bg-amber-500/[0.06]' : ''}`}><div className="flex justify-between gap-3"><div><p className="text-sm font-medium text-white">{m.materia}</p><p className="text-xs text-slate-400">{m.docente} · {m.sesiones} de {m.sesiones_programadas} sesiones registradas</p></div><span className={`text-sm font-semibold ${m.sesiones === 0 ? 'text-amber-300' : 'text-emerald-300'}`}>{pct(m.cobertura)} cobertura</span></div><p className="mt-2 text-xs text-slate-300">Asistencia: {m.asistencia_detalle.publicable ? pct(m.asistencia) : `${m.asistencia_detalle.texto} · muestra insuficiente`}{m.avance_sesion != null && m.sesiones >= datos.criterios.min_sesiones_porcentaje ? ` · Cumplimiento declarado: ${m.avance_sesion}%` : m.avance_sesion != null ? ` · Cumplimiento declarado: muestra insuficiente (${m.sesiones} ${m.sesiones === 1 ? 'sesión registrada' : 'sesiones registradas'})` : ''}</p>{m.ultimo_tema && <p className="mt-2 text-xs text-slate-300"><b>Tema reciente:</b> {m.ultimo_tema}</p>}{m.pendiente && <p className="mt-1 text-xs text-amber-300"><b>Pendiente:</b> {m.pendiente}</p>}</div>)}</div></article>)}</div></section>}
      <section className="glass overflow-hidden rounded-2xl"><header className="border-b border-white/10 px-5 py-4"><h2 className="font-semibold text-white">Alumnos con indicador académico</h2><p className="text-xs text-slate-300">La clasificación considera registros de asistencia y seguimientos abiertos.</p></header><div className="overflow-x-auto"><table className="w-full min-w-[820px] text-left text-sm"><thead className="text-xs uppercase text-slate-400"><tr><th className="px-5 py-3">Alumno</th><th>Grupo</th><th>Asistencias / registros</th><th>Faltas</th><th>Seguimientos</th><th>Clasificación</th></tr></thead><tbody className="divide-y divide-white/5">{datos.alumnos_atencion.map((a)=><tr key={a.alumno_id}><td className="px-5 py-3"><b className="text-white">{a.nombre}</b><small className="block text-slate-400">{a.matricula}</small></td><td>{datos.grupos.find((g)=>g.id===a.grupo_id)?.nombre}</td><td>{a.asistencias_registradas}/{a.registros}{a.asistencia != null ? ` · ${pct(a.asistencia)}` : ''}</td><td>{a.faltas}</td><td>{a.seguimientos_abiertos}</td><td><span className={`rounded-full px-2 py-1 text-xs font-semibold ${a.nivel==='PRIORITARIO'?'bg-red-500/15 text-red-300':a.nivel==='DATOS INSUFICIENTES'?'bg-slate-500/15 text-slate-300':'bg-amber-500/15 text-amber-300'}`}>{a.nivel}</span></td></tr>)}</tbody></table>{!datos.alumnos_atencion.length&&<p className="p-6 text-center text-sm text-slate-300">{datos.resumen.sesiones === 0 ? 'Aún no hay sesiones registradas para evaluar indicadores académicos.' : 'No hay alumnos con indicador académico en la selección.'}</p>}</div><details className="border-t border-white/10 bg-white/[0.02] px-5 py-4"><summary className="cursor-pointer text-xs font-semibold text-slate-300">Cómo se clasifican estos resultados</summary><div className="mt-3 grid gap-2 text-xs text-slate-300 md:grid-cols-3"><p><b className="text-slate-200">Datos insuficientes:</b> menos de 3 registros sin seguimiento.</p><p><b className="text-amber-300">Atención:</b> seguimiento abierto o asistencia menor a 85% con 3 sesiones.</p><p><b className="text-red-300">Prioritario:</b> dos seguimientos abiertos o asistencia menor a 80% con 5 sesiones.</p></div></details></section>
      {datos.sesiones_especiales.length > 0 && <section className="glass overflow-hidden rounded-2xl"><header className="border-b border-white/10 px-5 py-4"><h2 className="font-semibold text-white">Sesiones con registro extemporáneo o corregido</h2><p className="text-xs text-slate-400">Eventos que requieren contexto adicional para interpretar el reporte.</p>{resumenExtemporaneas && <p className="mt-3 rounded-lg border border-blue-500/20 bg-blue-500/[0.07] px-3 py-2 text-sm font-medium text-blue-200">{resumenExtemporaneas}</p>}</header><div className="divide-y divide-white/5">{datos.sesiones_especiales.map((s,i)=><div key={`${s.fecha}-${s.materia}-${i}`} className="px-5 py-3 text-sm"><b className="text-white">{s.materia}</b><span className="ml-2 text-xs text-slate-500">{fechaLegible(s.fecha)} · {datos.grupos.find((g)=>g.id===s.grupo_id)?.nombre} · {s.docente}</span><p className="mt-1 text-xs text-slate-300">{s.extemporanea ? `Registro extemporáneo${s.motivo_extemporaneo ? `: ${s.motivo_extemporaneo}` : ''}` : ''}{s.extemporanea && s.correcciones ? ' · ' : ''}{s.correcciones ? `${s.correcciones} movimiento(s) de corrección` : ''}</p></div>)}</div></section>}
      <div className={`grid gap-4 ${datos.incidencias.length ? 'xl:grid-cols-2' : ''}`}>
