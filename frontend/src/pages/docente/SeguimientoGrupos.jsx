...
export default function SeguimientoGrupos() {
  ...
  const detalleMuestra = datos?.total_clases
    ? `Basado en ${datos.total_clases} ${datos.total_clases === 1 ? 'clase' : 'clases'}${asistenciaGrupo.estado === 'MUESTRA_INSUFICIENTE' ? ' · muestra inicial' : ''}`
        ...
        {datos && (
          <>
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              {[
                ['Clases impartidas', datos.total_clases, 'Solo capturas concluidas con lista'],
                ['Alumnos', datos.total_alumnos, 'Inscritos actualmente'],
                ['Asistencia registrada', asistenciaGrupo.texto, detalleMuestra],
                ['Alumnos en alerta', datos.alumnos_en_alerta, datos.alumnos_en_alerta ? 'Requieren atención' : 'Sin alertas activas'],
              ].map(([label, value, detail]) => (
                <div key={label} className="glass rounded-xl p-4">
                  <p className={`text-2xl font-bold ${label === 'Asistencia registrada' ? asistenciaGrupo.clase : 'text-white'}`}>{value}</p>
              ...
              {datos.alumnos.map((a) => (
                <article key={a.alumno_id} className="glass rounded-2xl p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className={`shrink-0 rounded-full bg-slate-500/10 px-3 py-1 text-sm font-bold ${formatAsistencia(a.porcentaje_asistencia, datos.total_clases).clase}`}>
                      {formatAsistencia(a.porcentaje_asistencia, datos.total_clases).texto}
                      {formatAsistencia(a.porcentaje_asistencia, datos.total_clases).estado === 'MUESTRA_INSUFICIENTE' && <small className="ml-1 font-normal">· muestra inicial</small>}
                    </span>
                  </div>
              ...
              <table className="w-full min-w-[980px] text-left text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                  {datos.alumnos.map((a) => <tr key={a.alumno_id}>
                    ...
                    <td className={`font-semibold ${formatAsistencia(a.porcentaje_asistencia, datos.total_clases).clase}`}>
                      {formatAsistencia(a.porcentaje_asistencia, datos.total_clases).corto}
                      <span className="block text-[10px] font-normal text-slate-500">
                        {datos.total_clases ? `${datos.total_clases} ${datos.total_clases === 1 ? 'clase' : 'clases'}${formatAsistencia(a.porcentaje_asistencia, datos.total_clases).estado === 'MUESTRA_INSUFICIENTE' ? ' · muestra inicial' : ''}` : 'Sin clases impartidas'}
                      </span>
                    </td>
