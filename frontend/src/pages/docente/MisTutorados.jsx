...
export default function MisTutorados() {
  ...
  const TABS = [
    ...
    { id: "sesiones",       label: "Tutorías",        badge: null },
          ...
          <div className="overflow-hidden rounded-2xl border border-white/10 bg-slate-900/45">
            <div className="flex flex-col gap-3 border-b border-white/10 p-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-400">{alumnos.length} alumnos · {alumnos.filter(a => a.requiere_atencion).length} requieren atención · asistencia consolidada de todas sus materias</p>
              </div>
              <div className="flex w-full flex-col gap-2 sm:flex-row lg:w-auto">
                ...
                <select id="filtrar-tutorados" value={filtroAlumnos} onChange={e => setFiltroAlumnos(e.target.value)} className="input-dark sm:w-52">
                  ...
                  <option value="SIN_SESION">Sin tutoría registrada</option>
                ...
                <tbody className="divide-y divide-white/5">
                  {alumnosFiltrados.map(a => {
                    ...
                    return <tr key={a.id} className={`transition hover:bg-white/[0.035] ${a.requiere_atencion ? 'bg-amber-500/[0.025]' : ''}`}>
                      ...
                      <td className="px-4 py-3.5">{a.materias_riesgo ? <button type="button" onClick={() => navigate(`/expediente-academico?alumno=${a.id}&tab=asistencia`)} title={(a.detalle_materias_riesgo || []).map(m => `${m.materia}: ${m.asistencia != null ? `${m.asistencia}%` : 'sin porcentaje'}, ${m.faltas} ${m.faltas === 1 ? 'falta' : 'faltas'}${m.docente ? ` · ${m.docente}` : ''}`).join('\n')} aria-label={`${a.materias_riesgo} ${a.materias_riesgo === 1 ? 'materia con señal' : 'materias con señal'}. Abrir detalle de asistencia.`} className="inline-flex items-center gap-1.5 rounded-full border border-amber-500/25 bg-amber-500/10 px-2.5 py-1 text-xs font-semibold text-amber-700 transition hover:border-amber-500/40 hover:bg-amber-500/15 focus:outline-none focus:ring-2 focus:ring-amber-500/40"><span className="h-1.5 w-1.5 rounded-full bg-amber-500" aria-hidden="true" />{a.materias_riesgo} {a.materias_riesgo === 1 ? 'materia' : 'materias'}<span aria-hidden="true">›</span></button> : <span className="text-slate-600">—</span>}</td>
                      ...
                      <td className="px-5 py-3.5 text-right"><button type="button" onClick={() => navigate(`/expediente-academico?alumno=${a.id}`)} className="rounded-lg border border-emerald-500/30 px-3 py-2 text-xs font-semibold text-emerald-300 hover:bg-emerald-500/10">Ver expediente ›</button></td>
                    </tr>;
                  })}
                </tbody>
              </table>
