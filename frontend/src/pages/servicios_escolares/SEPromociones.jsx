...
export default function SEPromociones() {
  ...
  const abrirGrupo = grupo => {
    ...
    setForm({ resolucion: 'PROMOVIDO', cuatrimestre_destino: Math.min(Number(primero.cuatrimestre_origen) + 1, 12), grupo_destino: primero.grupo_origen, observaciones: '' });
  };
  ...
  return <AdminLayout><div className="space-y-5 p-6">
    ...
    <section className="glass overflow-hidden rounded-2xl"><div className="flex flex-wrap gap-3 border-b border-white/10 p-4"><input className="input-dark min-w-64 flex-1" placeholder="Buscar alumno, matrícula o carrera…" value={filtro} onChange={e => setFiltro(e.target.value)} /><div className="flex rounded-xl bg-white/5 p-1"><button onClick={() => setVista('GRUPOS')} className={`rounded-lg px-4 py-2 text-xs font-semibold ${vista === 'GRUPOS' ? 'bg-emerald-600 text-white' : 'text-slate-400'}`}>Por grupos</button><button onClick={() => setVista('ALUMNOS')} className={`rounded-lg px-4 py-2 text-xs font-semibold ${vista === 'ALUMNOS' ? 'bg-emerald-600 text-white' : 'text-slate-400'}`}>Por alumnos</button></div></div>{vista === 'GRUPOS' ? <div className="divide-y divide-white/5">{grupos.map(g => { const pendientes = g.alumnos.filter(a => a.resolucion === 'PENDIENTE').length; const aplicados = g.alumnos.filter(a => a.estado === 'APLICADA').length; const abierto = expandidos[g.clave]; return <div key={g.clave}><div className="flex flex-wrap items-center justify-between gap-3 p-5"><button onClick={() => setExpandidos(x => ({...x,[g.clave]:!x[g.clave]}))} className="text-left"><b className="text-white">{g.origen} · {g.carrera}</b><p className="mt-1 text-xs text-slate-400">{g.alumnos.length} alumnos · {pendientes} pendientes · {aplicados} aplicados</p></button><div className="flex gap-2"><button onClick={() => setExpandidos(x => ({...x,[g.clave]:!x[g.clave]}))} className="rounded-lg border border-white/10 px-3 py-2 text-xs text-slate-300">{abierto ? 'Ocultar' : 'Revisar alumnos'}</button><button disabled={!pendientes} onClick={() => abrirGrupo(g)} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white disabled:opacity-40">Resolver pendientes</button></div></div>{abierto && <div className="overflow-x-auto border-t border-white/5 bg-black/10"><table className="w-full text-left text-sm"><tbody className="divide-y divide-white/5">{g.alumnos.map(a => <tr key={a.inscripcion_id}><td className="p-4"><b className="text-white">{a.alumno}</b><br/><span className="text-xs text-slate-500">{a.matricula}</span></td><td className="text-slate-300">{a.resolucion.replaceAll('_',' ')}</td><td className="text-slate-300">{['PROMOVIDO','REPITE'].includes(a.resolucion) ? `${a.cuatrimestre_destino}° ${a.grupo_destino}` : '—'}</td><td className="pr-4 text-right"><button disabled={a.estado === 'APLICADA'} onClick={() => abrir(a)} className="rounded-lg border border-blue-500/30 px-3 py-2 text-xs font-semibold text-blue-400 disabled:opacity-30">{a.estado === 'APLICADA' ? 'Aplicada' : 'Resolver excepción'}</button></td></tr>)}</tbody></table></div>}</div>})}{!cargando && !grupos.length && <p className="p-8 text-center text-sm text-slate-500">No hay grupos para mostrar.</p>}</div> : <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-white/[.03] text-xs uppercase text-slate-500"><tr><th className="p-4">Alumno</th><th>Origen</th><th>Resolución</th><th>Destino</th><th></th></tr></thead><tbody className="divide-y divide-white/5">{filas.map(a => <tr key={a.inscripcion_id}><td className="p-4"><b className="text-white">{a.alumno}</b><br/><span className="text-xs text-slate-500">{a.matricula} · {a.carrera}</span></td><td className="text-slate-300">{a.origen}</td><td><span className="rounded-full bg-white/5 px-2 py-1 text-xs text-slate-300">{a.resolucion.replaceAll('_',' ')}</span></td><td className="text-slate-300">{['PROMOVIDO','REPITE'].includes(a.resolucion) ? `${a.cuatrimestre_destino}° ${a.grupo_destino}` : '—'}</td><td className="pr-4 text-right"><button disabled={a.estado === 'APLICADA'} onClick={() => abrir(a)} className="rounded-lg border border-blue-500/30 px-3 py-2 text-xs font-semibold text-blue-400 disabled:opacity-30">{a.estado === 'APLICADA' ? 'Aplicada' : 'Resolver'}</button></td></tr>)}</tbody></table></div>}</section></>}
